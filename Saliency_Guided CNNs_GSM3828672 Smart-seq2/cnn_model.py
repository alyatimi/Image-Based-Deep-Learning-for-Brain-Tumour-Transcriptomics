...
def evaluate_model_full(model, dataloader, class_names):
    ...
    auc_weighted = roc_auc_score(all_labels, all_probs, multi_class='ovr', average='weighted')  # ← ADD THIS
...
def plot_training_history(train_losses):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Training Loss per Fold GSM3828672")
    plt.legend()
    plt.tight_layout()
...
def plot_confusion_matrix(cm, class_names, fold=None):
    ...
    title = f"Confusion Matrix GSM3828672"
    if fold is not None:
        title += f" (Fold {fold + 1})"
    plt.title(title)
    plt.tight_layout()
